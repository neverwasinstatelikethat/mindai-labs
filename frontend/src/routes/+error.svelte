  ...
  const scene = $derived.by<Scene>(() => {
    ...
    if (status === 403) {
      return {
        ...
        problem:
          'Разбор предложений эволюции, аудит и закрытые данные открываются при расширенном доступе. Остальные разделы работают как обычно, и показаний корпуса из-за этого отказа не пропало.',
